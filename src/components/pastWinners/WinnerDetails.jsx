...
const WinnerDetails = ({ data }) => {
  ...
  useEffect(() => {
    ...
    const lastBandSideEnd = 3 + (bands.length - 1) * WIPE_INTERVAL + SIDE_DURATION;
    ...
    const ctx = gsap.context(() => {
      ...
      tl.to(imageRef.current, {
        z: 40, scale: 0.8, x: window.innerWidth * 0.05, rotateY: 0,
        ease: "none", duration: 2,
      }, 0);

      tl.to(imageRef1.current, {
        height: "45vw", width: "45vw", z: 40, scale: 1, rotateY: 0,
        ease: "none", duration: 2,
      }, 0);

      tl.to(imageRef2.current, {
        z: 40, scale: 1, x: -window.innerWidth * 0.23, rotateY: 0,
        ease: "none", duration: 2,
      }, 0);

      tl.to(imageRef3.current, {
        z: 40, scale: 1, x: -window.innerWidth * 0.46, rotateY: 0,
        ease: "none", duration: 2,
      }, 0);

      tl.to(imageRef4.current, {
        z: 40, scale: 1, x: -window.innerWidth * 0.5, rotateY: 0,
        ease: "none", duration: 2,
      }, 0);

      tl.to(textRef.current, {
        y: -window.innerWidth * 0.5, ease: "none", duration: 2,
      }, 0);
      ...
      transitions.forEach((band, i) => {
        ...
        if (imgEl) {
          tl.to(imgEl, {
            ...
            duration: WIPE_DURATION, ease: "none",
          }, pos);
        }
        ...
        if (prevText) {
          tl.to(prevText, {
            ...
            duration: TEXT_DURATION, ease: "power1.inOut",
          }, pos);
        }

        if (nextText) {
          tl.to(nextText, {
            top: "50%", yPercent: -50, opacity: 1,
            duration: TEXT_DURATION, ease: "power1.inOut",
          }, pos);
        }
      });
    ...
    >
      ...
      <div className={styles.WinnerDetailsContentWrapper}>
        <div ref={textRef} className={styles.WinnerDetailsLeftSection}>
          <img src={data.BandNameTitle} alt="Band Title" className={styles.BandName} />
          <img src={data.DecadeTitle} alt="Decade Title" className={styles.DecadeTitle} />
          <p className={styles.Description}>{data.description}</p>
        </div>

        <div className={styles.WinnerDetailsRightSection}>
          <div ref={imageRef} className={styles.AlbumStack}>
            <img className={styles.img1} src={baseBand?.albumStack?.[3]} alt="" ref={imageRef1} />
            <img className={styles.img2} src={baseBand?.albumStack?.[1]} alt="" ref={imageRef2} />
            <img className={styles.img3} src={baseBand?.albumStack?.[2]} alt="" ref={imageRef3} />
            <img className={styles.img4} src={baseBand?.albumStack?.[0]} alt="" ref={imageRef4} />

            <img src={baseImg} className={styles.circle} alt="" ref={circleRef} />
            ...
            <div ref={circlesmallref} className={styles.smallcircle}>
              {bands.map((band, i) => (
                <div
                  ...
                  className={i === 0 ? styles.circleTextTop : styles.circleTextBottom}
                >
            ...
            {transitions.map((band, i) => {
              ...
              return (
                <React.Fragment key={`wipe-${band.id}`}>
                  <div ref={(el) => (swipeImgRefs.current[i] = el)} className={styles.swipeImg}>
                    <img src={wipeImg} alt="" />
                  </div>
                  <div ref={(el) => (swipeCircleRefs.current[i] = el)} className={styles.swipeCircle}>
                    <img src={wipeImg} alt="" />
                  </div>
                </React.Fragment>
              );
